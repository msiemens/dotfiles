...
function myPrompt() {
    ...
    PROMPT_RBRACE="]"
    PROMPT_SPACER=""
    PROMPT_SMILEY_HAPPY="\xF0\x9F\x99\x82"
    PROMPT_SMILEY_SAD="\xF0\x9F\x99\x81"

	PS1="\`if [ \$? = 0 ];
	    then
		echo -e '$GREEN$PROPMT_LBRACE $DARK_GRAY\u$GREEN $PROMPT_RBRACE$PROMPT_SPACER$PROPMT_LBRACE$DARK_GRAY$VENV $BROWN\w$STATUS$GREEN $PROMPT_RBRACE $PROMPT_SMILEY_HAPPY\n$PROMPT_CHAR$DEFAULT_COLOR ';
	    else
		echo -e '$LIGHT_RED$PROPMT_LBRACE $DARK_GRAY\u$LIGHT_RED $PROMPT_RBRACE$PROMPT_SPACER$PROPMT_LBRACE$DARK_GRAY$VENV $BROWN\w$LIGHT_RED$STATUS$LIGHT_RED $PROMPT_RBRACE $PROMPT_SMILEY_SAD\n$PROMPT_CHAR$DEFAULT_COLOR ';
	    fi; \`"
}
